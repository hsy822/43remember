...
class FormVictim extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  createVictim = async e => {
    e.preventDefault();
    this.handleClose();
    const { contract, coinbase, victimName, victimAddr } = this.state;
    let result = await contract.methods
      .addVictim(victimName, victimAddr)
      .send({ from: coinbase });
    console.log(result);
    this.setState({
      resultIdx: result.events.victimAdded.returnValues[0],
      resultName: result.events.victimAdded.returnValues[1],
      resultAddr: result.events.victimAdded.returnValues[2],
      resultCreateTime: this.convert(result.events.victimAdded.returnValues[3])
    });
        ...
        <Paper className={this.state.classes.paper}>
          ...
          <form className={this.state.classes.form}>
            <FormControl margin="normal" required fullWidth>
              <InputLabel htmlFor="victimName">이름</InputLabel>
              <Input
                id="victimName"
                name="victimName"
                autoComplete="victimName"
                autoFocus
                onChange={this.handleChange("victimName")}
              />
            </FormControl>
            <FormControl margin="normal" required fullWidth>
              <InputLabel htmlFor="victimAddr">주소</InputLabel>
              <Input
                name="victimAddr"
                id="victimAddr"
                autoComplete="victimAddr"
                autoFocus
            ...
            <div>
              ...
              >
                ...
                <DialogActions>
                  ...
                  <Button onClick={this.createVictim} color="primary" autoFocus>
                    확인
                  </Button>
                </DialogActions>
              </Dialog>
...
FormVictim.propTypes = {
  classes: PropTypes.object.isRequired
};

export default withStyles(styles)(FormVictim);
